...
type Props = {
  ...
  longer? : boolean; // half-assed patch to make the '0' longer
}
...
function calculation(firstNum : number, secondNum : number, op : Operations) : number  {
    ...
    console.log("Calculating: ", firstNum, op, secondNum);
...
function stackBack(val : Operations | string){ // takes in operation, compresses the stack (nothing is returned)
    ...
    if (isOp(val)){ // is operation
        ...
        if (stack.length == 1){ // only number exists in the stack
            if (val === "="){
                nextRefresh = true;
                return;
            }
        ...
        else{ // has number and operation existing in the stack
            let firstNum : number = Number(stack[0]);
            let middleOp = stack[1];

            if (isOp(stack[stack.length-1])){ // accounts for case: two consec ops (never allow more than 2 ops)
                // prioritise the later one
                console.log("Consecutive Ops detected")
                stack.pop();
                stack.push(val);
                return;
            }
            ...
            if (typeof answer === "string"){ // for div by 0
                return; // this actually never happens
            }
            else {
                stack = [answer.toString()];

                if (val != "=") {
                    stack.push(val);  // pushes the current op into the stack
                    // nextRefresh = true;
                }
                else nextRefresh = true; 
      ...
      currNum = answer;
    ...
    else if (!Number.isNaN(Number(val))){ // is a digit
        console.log(stack[0]);
        ...
        else if (stack.length == 1 || stack.length == 3){ // currently on number mode: compress the number into one block
