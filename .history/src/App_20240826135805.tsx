...
type Props = {
  ...
  setACDis? : React.Dispatch<React.SetStateAction<"AC" | "C">>
}
...
function refresh(props : Props){
  ...
  const setACDis = props.setACDis;
  if (typeof setACDis === "undefined"){
    // other buttons pressed
    //dont apply
    console.log("its undefined welp")
  }
  else{
    // ac pressed
...
function startWithRefresh(setACDis : React.Dispatch<React.SetStateAction<"AC" | "C">>){
  setACDis("AC");
}
...
function Buttons( { val, colour, setDisplay, setACDis}  : Props){
  ...
  return <button className = {`Buttons ${colour}`} onClick = {() => handleClick({ val, colour, setDisplay, setACDis })}>
    {val}
  </button>
}
...
export default function Calc() {
  ...
  return (
    <main>
      ...
      <div className = "row">
        <Buttons val={ACdis} colour = 'darkgrey' setDisplay = {setDisplay} setACDis={setACDis} /> 
