...
function Buttons( {val, colour, setDisplay, longer}  : Props){
    ...
    function handleClick(props : Props){
      ...
      if (val === "AC" || val === "C"){
        ...
        stackBack(val);
        if (isOp(val)) setDisplay(currNum.toString());
        if (!Number.isNaN(stack[stack.length-1])) setDisplay(stack[stack.length-1]);
      }
    ...
    //     // console.log(currNum);
    //     setDisplay(stack[stack.length-1])
    //       // setDisplay(currNum.toString()); // cannot use the last
